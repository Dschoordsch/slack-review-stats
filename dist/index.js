...
`;
const calculateTimeToReview = (request, review) => {
    const timeToReview = review.valueOf() - request.valueOf();
    const requestWeek = (0, dayjs_1.default)(request).isoWeek();
    const reviewWeek = (0, dayjs_1.default)(review).isoWeek();
    const weekends = reviewWeek - requestWeek;
    return timeToReview - weekends * (0, ms_1.default)('2d');
};
const padRight = (str, width) => {
    const length = Math.min(str.length, width);
    return str.slice(0, length) + ' '.repeat(width - length);
};
const formatRow = (values, format) => {
    const rows = [];
    do {
        const row = [];
...
const safeMs = (val) => typeof val === 'number' && isFinite(val) ? (0, ms_1.default)(val) : undefined;
class ReviewStats {
    constructor(options) {
        ...
        this.pushToSlack = (body) => __awaiter(this, void 0, void 0, function* () {
            const response = yield (0, node_fetch_1.default)(this.slackWebhook, {
                method: 'POST',
                ...
                pr.timelineItems.nodes.forEach((item) => {
                    ...
                    if (item.__typename === 'PullRequestReview') {
                        ...
                        if (!reviewerStats[login]) {
                            reviewerStats[login] = Object.assign(Object.assign({}, item.author), { timesToReview: [], reviewStates: [], comments: 0, reviewedPRs: 0 });
                        }
                    ...
                    if (item.__typename === 'MergedEvent') {
                        ...
                        if (login !== pr.author.login) {
                            if (!reviewerStats[login]) {
                                reviewerStats[login] = Object.assign(Object.assign({}, item.actor), { timesToReview: [], reviewStates: [], comments: 0, reviewedPRs: 0 });
                            }
                            const request = requestedReviewers[login];
                            if (request) {
